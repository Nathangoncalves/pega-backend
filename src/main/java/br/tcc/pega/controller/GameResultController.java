...
public class GameResultController {
    ...
    public ResponseEntity<GameResultDto> save(@Valid @RequestBody GameResultDto dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(gameResultService.save(dto));
    }

    @GetMapping("/student/{studentId}")
    @Operation(summary = "Histórico de resultados de um aluno (ordem decrescente por data)")
    public ResponseEntity<List<GameResultDto>> getByStudent(@PathVariable Long studentId) {
        return ResponseEntity.ok(gameResultService.findByStudentId(studentId));
    }

    @GetMapping("/{id}")
    @Operation(summary = "Buscar resultado específico por ID")
    public ResponseEntity<GameResultDto> getById(@PathVariable Long id) {
